    ...
    <main className="section">
      <div className="container">
        <div className="notice">
          <strong>Admin page error:</strong> {message}
        </div>
      </div>
    </main>
  );
...
export default async function AdminDashboard() {
  ...
  try {
    ...
    const [productsResult, ordersResult, latestOrdersResult, lowStockResult] = await Promise.all([
    ...
    const firstError = productsResult.error || ordersResult.error || latestOrdersResult.error || lowStockResult.error;
    if (firstError) {
      return <AdminError message={firstError.message} />;
    }
    ...
    const salesTotal = latestOrders.reduce((sum: number, order: any) => sum + Number(order.total_amount || 0), 0);
        ...
        <div className="container admin-shell">
          ...
          <section>
            ...
            <div className="kpi-grid">
              <div className="kpi"><span className="muted">Products</span><strong>{productCount}</strong></div>
              <div className="kpi"><span className="muted">Orders</span><strong>{orderCount}</strong></div>
              <div className="kpi"><span className="muted">Latest orders total</span><strong>{formatMoney(salesTotal)}</strong></div>
              <div className="kpi"><span className="muted">Low stock</span><strong>{lowStock.length}</strong></div>
            </div>
                ...
                <tbody>
                  {latestOrders.map((order: any) => (
                    <tr key={order.order_number}>
                      ...
                      <td>{Array.isArray(order.customers) ? order.customers[0]?.full_name || '-' : order.customers?.full_name || '-'}</td>
              ...
              <table className="table">
                ...
                <tbody>
                  {lowStock.map((variant: any) => (
                    <tr key={variant.sku}>
                      <td>{Array.isArray(variant.products) ? variant.products[0]?.name || '-' : variant.products?.name || '-'}</td>
                      <td>{variant.sku}</td>
